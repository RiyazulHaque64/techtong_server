...
const SendOTP_1 = __importStar(require("../../utils/SendOTP"));
...
const createOTP = (data) => __awaiter(void 0, void 0, void 0, function* () {
    var _a;
    const generatedOTP = (0, SendOTP_1.OTPGenerator)();
    ...
    let emailResponse;
    if (data.email) {
        emailResponse = yield (0, sendEmail_1.default)(data.email, emailBody);
    }
    const SMSResponse = yield (0, SendOTP_1.default)(data.contact_number, SMSBody);
    ...
    const result = yield prisma_1.default.userOTP.create({
        data: {
            name: data.name,
...
const register = (data) => __awaiter(void 0, void 0, void 0, function* () {
    const storedOTP = yield prisma_1.default.userOTP.findFirst({
        where: {
            otp: Number(data.otp),
        },
    });
    if (!storedOTP) {
        throw new ApiError_1.default(http_status_1.default.FORBIDDEN, "OTP not matched");
    }
    const verifiedOTP = yield (0, SendOTP_1.verifyOTP)(Number(data.otp), storedOTP.otp, Number(storedOTP.expires_at));
    if (verifiedOTP.success === false) {
        throw new ApiError_1.default(http_status_1.default.FORBIDDEN, verifiedOTP.message);
    }
...
const login = (credential) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    if (!checkPassword) {
        throw new ApiError_1.default(http_status_1.default.FORBIDDEN, "Email/Contact number or password is invalid");
    }
    const jwtPayload = {
        id: user.id,
    ...
    const refreshToken = (0, jwtHelpers_1.generateToken)(jwtPayload, config_1.default.jwt_refresh_secret, config_1.default.jwt_refresh_expiresin);
    return {
        id: user.id,
        name: user.name,
        email: user.email,
...
const getAccessToken = (token) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    const verifiedUser = (0, jwtHelpers_1.verifyToken)(token, config_1.default.jwt_refresh_secret);
    const user = yield prisma_1.default.user.findFirst({
        where: {
            id: verifiedUser.id,
            status: client_1.UserStatus.ACTIVE,
...
const forgotPassword = (email_or_contact_number) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    const generatedPassword = (0, generatePassword_1.generatePassword)(6);
    ...
    let emailResponse;
    if (user.email) {
        emailResponse = yield (0, sendEmail_1.default)(user.email, emailBody);
    }
    const SMSBody = `Dear ${user.name || "customer"}, your new password is: ${generatedPassword} \nTECHTONG`;
    const SMSResponse = yield (0, SendOTP_1.default)(user.contact_number, SMSBody);
    if (((_b = emailResponse === null || emailResponse === void 0 ? void 0 : emailResponse.accepted) === null || _b === void 0 ? void 0 : _b.length) === 0 && SMSResponse.success === false)
        throw new ApiError_1.default(http_status_1.default.INTERNAL_SERVER_ERROR, "Failed to send new password to user email and contact number");
